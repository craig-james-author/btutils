...
// Convert the volume, range is 0 to 100 (percent).
...
void BtUtils::_doVolumeFadeOut() {
  ...
  _actualVolume = 0;
  _MP3player->setVolume((uint8_t)254);
...
void BtUtils::startTrack(int trackNumber) {
  ...
  if (_fadeInTime > 0) {
    _setActualVolume(0);	// fade-in: start with zero volume
  } else {
