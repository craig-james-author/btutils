// Getting near (proximity) to the sensor starts the track. Moving away it
// stops it. Getting near the same sensor, resumes where stopped. Proximity
// to a new sensor, starts it. Returning to a previous sensor resumes where
// it left off.  Needs one second of something at the start of track -- can
// be silence or a sound such a waves, bell, or something that relates to
...
void loop() {
  ...
  if (touchStatus == NEW_TOUCH) {
    ...
    if (playerStatus == IS_PAUSED && trackNumber == lastPlayed) {
      bt->resumeTrack();
      bt->log_action("Resume:", lastPlayed);
    } else {
	...
	trackPosition[lastPlayed] = 0;
      ...
      bt->log_action("Continue: ", trackNumber);
    }
  }
  ...
  // the last track-position value.
    ...
    if (playerStatus == IS_STOPPED) {
      trackPosition[lastPlayed] = 0;
      bt->stopTrack();
      bt->log_action("Track ended: ", lastPlayed);
    } else {
      trackPosition[lastPlayed] += currentLocation;
      bt->pauseTrack();
      bt->log_action("Pause: ", lastPlayed);
    }
  }

  // This last section turns the LED off if the end of the track is reached
