// Proximity to the sensor starts the track. Moving away stops it. Proximity to the same
// sensor, resumes where stopped. Proximity to a new sensor starts that track. Proximity
// to the previous sensor resumes where it last stopped.  Needs one second of something at
// the start of track -- can be silence or a sound such a waves, bell, or something that
// relates to the content of the track.
...
void setup() {
  ...
  bt->setFadeInTime(500);    // 1/2 second fade-in
  ...
  bt->setTouchReleaseThreshold(2,1);
...
void loop() {
  ...
  // If a new proximity is detected:
